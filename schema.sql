...
begin
  ...
  if deleted_count=0 then raise exception 'Không tìm thấy tin nhắn hoặc tin đã bị xóa'; end if;
  return true;
end; $$;
grant execute on function public.admin_delete_message(text,text,uuid) to anon,authenticated;
